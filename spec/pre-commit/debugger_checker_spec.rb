...
describe PreCommitHandler do
  ...
  describe "es6 javascript files" do
    before :each do
      allow_any_instance_of(Object).to receive(:`).with("git diff --name-only --cached").and_return("file.js.es6")
    end

    describe "that are valid" do
      before :each do
        allow_any_instance_of(Object).to receive(:`).with("git show :file.js.es6").and_return(" some js code ")
      end
    ...
    describe "with debugger" do
      before :each do
        allow_any_instance_of(Object).to receive(:`).with("git show :file.js.es6").and_return("dsafa debugger")
      end
      ...
      it "will fail" do
        ...
        ****************************************
        ...
        File \e[31m./file.js.es6\e[39m contains debugger
